...
public class json {
	...
	public JSONObject sendRequest(String url) throws JSONFailureException
	{
		...
		try {
			...
			throw new JSONFailureException(new ArrayList<String>(){{add("Could not connect to server");}});
		...
		if(data == null)
			throw new JSONFailureException(new ArrayList<String>(){{add("Could not parse response");}});
