...
public class ChildSettingsTab extends JPanel {
	...
	public ChildSettingsTab(Settings settings) {
		...
		try {
			...
			Vector<String> columnNames = new Vector<String>(
					Arrays.asList(new String[] { "Name", "Date of Birth",
							"Age", "Level" }));
				...
				public Component getTableCellRendererComponent(JTable table,
					...
					if (col == 1 && row != -1) {
						return super.getTableCellRendererComponent(table,
								new SimpleDateFormat("d MMM yyyy").format(obj),
			...
			// Limit the number of characters that can be input into the name
			// input field
			((AbstractDocument) nameInput.getDocument())
					.setDocumentFilter(new DocumentLengthFilter(20));
			...
			// Sets the default selections to the current values for selected
			// child
			setSelections();
			...
			progress.addActionListener(new PressProgress(settings, this,
					childSelect));
			...
			c.gridx = 2;
			c.gridwidth = 2;
			add(progress, c); // The button to display progress of selected
								// child
	 ...
	 * 
	 * @param childSelect
	 *            the child selection combo box
	 * @param level
	 *            the level selection combo box to be populated
	 */
	...
	/**
	 * Sets the default selections for all of the birthday drop downs, and the
	 * level drop down to match the currently selected child.
	 * 
	 */
	public void setSelections() {
		try {
			// Get the currently selected child's birthday
			Date birth = ProgenyService.getProgenies()
					.get(childSelect.getSelectedIndex()).getBirthDate();
			...
			// Get the current level achieved by the child and set the level
			// drop down to it
	...
	/**
	 * Populates the birth-day drop down with the correct day range for selected
	 * year and month
	 * 
	 * @param year
	 ...
	 */
	public static void updateDaysInMonth(JComboBox<String> year,
			JComboBox<String> month, JComboBox<String> day) {
		...
		GregorianCalendar cal = new GregorianCalendar(yr,
				(month.getSelectedIndex() + 1), 1);
		...
		// If the old selection is in the new range and was in fact a selection,
		// set the selection back to that day
	...
	public void updateChild() {
		ConfirmDialogue confirm = new ConfirmDialogue(
				"Warning! Updating a child's information is irreversible"
						+ " and data may be lost. Press Cancel to abort.",
				"Warning!", 2);
		if (confirm.getConfirmation()) {
			...
			try {
				...
				int yyyy = 0;
				try {
					if (year2.getItemCount() > 0
							&& year2.getSelectedIndex() > -1) {
						yyyy = Integer.parseInt((String) year2
								.getSelectedItem());
				...
				if (errors.isEmpty()) {
					// If the date of birth has changed, update progeny's
					// birthday
					...
					new GeneralDialogue("Child successfully updated.",
							"Success!", 3);
				} else {
	...
	public void addChild() {
		...
		try {
			...
			// Actually add the new child (with default level time of 30
			// seconds)
	...
	public void removeProgeny() {
		ConfirmDialogue confirm = new ConfirmDialogue(
				"Warning! Removing a child is irreversible,"
						+ " their game data will be lost! Press Cancel to abort.",
				"Warning!", 2);
		if (confirm.getConfirmation()) {
			...
			try {
				...
				if (toRemove.getFirstName().equals(
						childSelect.getSelectedItem())) {
...
class PressProgress implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			ChildProgress screen = new ChildProgress(settingsPane,
					childSettings, child);
...
class MonthSelected implements ActionListener {
	 ...
	 *            the birth-day drop down.
	 */
	public MonthSelected(JComboBox<String> year, JComboBox<String> month,
	...
	public void actionPerformed(ActionEvent e) {
		...
		// Update the list of days in the day drop down to match the date range
...
class YearSelected implements ActionListener {
	 ...
	 *            the birth-day drop down.
	 */
	public YearSelected(JComboBox<String> year, JComboBox<String> month,
			JComboBox<String> day) {
		super();
		this.year = year;
		this.month = month;
